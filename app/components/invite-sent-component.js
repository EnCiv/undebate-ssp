...
export default function InviteSentComponent({ invites }) {
    ...
    const invitesArr = Object.values(invites || {})
    if (invitesArr && invitesArr.length) {
        const sortedInvites = invitesArr
            .slice()
                    ...
                    <span className={classes.textSpan}>
                        ...
                        {sortedInvites.length > 1 ? (
                            <div className={classes.tooltip}>
                                <div>Previous Invites:</div>
                                {sortedInvites.slice(1).map(invite => {
                                    return <div key={i++}>{getSentText(invite.sentDate)}</div>
                                })}
                            </div>
                        ) : (
                            ''
        ...
        return <div className={classes.container}>No Invites Sent Yet</div>
    }
}
